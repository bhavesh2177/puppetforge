...
class kubernetes_v1_10_0::cluster_roles (
  Optional[Boolean] $controller = $kubernetes_v1_10_0::controller,
  Optional[Boolean] $worker = $kubernetes_v1_10_0::worker,
  ...
  if $controller {
    kubernetes_v1_10_0::kubeadm_init { $node_name:
      ignore_preflight_errors => $preflight_errors,
      }
    }

  if $worker {
    kubernetes_v1_10_0::kubeadm_join { $node_name:
